...
function showBiasDetectorOverlay(data) {
  ...
  function showTabContent(tabId) {
    ...
    if (targetElement) {
      ...
      console.error(`Tab content with ID bias-detector-tab-${tabId} not found`);
    }
  }
  
  // Extra setup to ensure tab buttons work correctly
  function setupTabEventListeners() {
    // First, log all tab buttons and their content for debugging
    console.log("Setting up tab event listeners");
    console.log("Tabs found:", document.querySelectorAll('.bias-detector-tab').length);
    console.log("Tab contents found:", document.querySelectorAll('.bias-detector-tab-content').length);
    
    // Clear any existing event listeners by removing and adding similar elements
    document.querySelectorAll('.bias-detector-tab').forEach(button => {
      // Create a completely new button element
      const newButton = document.createElement('button');
      newButton.className = button.className;
      newButton.textContent = button.textContent;
      newButton.setAttribute('data-tab-id', button.textContent.toLowerCase().replace(/\s+/g, ''));
      
      // Replace the old button
      button.parentNode.replaceChild(newButton, button);
    });
    
    // Now add fresh event listeners to all tab buttons
    const allTabButtons = biasDetectorOverlay.querySelectorAll('.bias-detector-tab');
    
    allTabButtons.forEach(button => {
      // Get the tab ID from text content and map it correctly
      let tabId = button.getAttribute('data-tab-id') || button.textContent.toLowerCase().replace(/\s+/g, '');
      
      // Map special cases
      if (tabId === 'biasanalysis') tabId = 'bias';
      ...
      button.addEventListener('click', function(event) {
        ...
        document.querySelectorAll('.bias-detector-tab-content').forEach(content => {
          content.classList.remove('active');
        });
        
        // Activate the correct content tab
        const tabContentId = `bias-detector-tab-${tabId}`;
        ...
        if (targetContent) {
          ...
          // Log all tab content IDs for debugging
          document.querySelectorAll('.bias-detector-tab-content').forEach(content => {
            console.log(`Available tab content: ${content.id}`);
          });
        }
      }, true); // Use capture phase for event listener
    });
